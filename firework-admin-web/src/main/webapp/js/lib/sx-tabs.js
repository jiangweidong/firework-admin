(function () {
    ...
    module.directive('sxTabs', ['$q', '$rootScope', '$http', '$controller', '$compile', '$templateCache', '$timeout', '$tabsConsts',
        function ($q, $rootScope, $http, $controller, $compile, $templateCache, $timeout, $tabsConsts) {
            return {
                scope: {
                ...
                link: function (scope, element) {
                    ...
                    var _compileTabContent = function (tab, isInit, callback) {
                        if (!tab.$scope || !tab.$controller || isInit) {
                            tab.$scope = scope.$new();
                            tab.$scope.$context = {
                                data: scope.$context,
                                behavior: {
                    ...
                    scope.switchTab = function (options, callback) {
                        ...
                        if (id && (
                            !scope.activeTab || !scope.activeTab.id ||
                                    ...
                                    _toggleCover(true, function () {
                                        ...
                                        _performLeaving(scope.activeTab, tab, byTabDisabled, function (valid) {
                                            if (valid) {
                                                ...
                                                _compileTabContent(tab, isInit, function () {
                                                    ...
                                                    _toggleCover(true, function () {
                                                        ...
                                                        _performEntering(fromTabId, tab, function () {
                                                            _toggleCover(false, function () {
                                                                ...
                                                                if(isInit)
                                                                    $rootScope.$broadcast(tab.id + '-init', tab);

                                                                $rootScope.$broadcast(tab.id + '-active', tab);
                                                                return callback(true);
                                                            });
                                                        });
                                                    });
                    ...
                    // Add by Daniel Kong
                    scope.$on('TAB_ENABLE', function(event, tabId){
                    ...
                    var _findLandingTab = function () {
                        ...
                        while (i <= scope.$tabsOrder.length - 1) {
                            ...
                            if (scope.$tabs[scope.$tabsOrder[i].id].isActive &&
                                scope.$tabs[scope.$tabsOrder[i].id].enabled && !firstActiveTabId) {
